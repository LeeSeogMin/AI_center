-- AI 정책연구소 게시판 테이블 생성 스크립트 (관리자 기능 포함)
-- Supabase SQL Editor에서 실행하세요

-- 1. user_roles 테이블 생성 (사용자 역할 관리)
...
-- 11. attachments 테이블 RLS 정책
CREATE POLICY "Attachments are viewable by everyone" ON attachments
  FOR SELECT USING (true);

-- 관리자만 첨부파일 관리 가능
CREATE POLICY "Only admins can manage attachments" ON attachments
  FOR ALL USING (is_admin(auth.uid()));

-- 12. 신규 사용자 자동 역할 부여 함수
...
-- 17. 관리자 계정 생성 안내
-- 첫 관리자를 수동으로 지정하려면 회원가입 후 아래 쿼리 실행:
-- UPDATE user_roles SET role = 'admin' WHERE user_id = '사용자-UUID';
--
-- 또는 이메일로 관리자 지정:
-- UPDATE user_roles
-- SET role = 'admin'
